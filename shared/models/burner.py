...
class BurnerAssessment(Base, TimestampMixin):
    ...
    identifier_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("identifiers.id", ondelete="CASCADE"), nullable=False, unique=True, index=True
    )
    ...
    carrier_category: Mapped[str | None] = mapped_column(String(50), nullable=True)  # voip, prepaid, mobile, unknown
    ...
    signals: Mapped[dict] = mapped_column(JSONB, default=dict, nullable=False)  # individual signal scores
