...
PYTHON ?= $(shell if [ -x .venv/bin/python ]; then echo .venv/bin/python; elif command -v poetry >/dev/null 2>&1; then echo "poetry run python"; else echo python3; fi)
...
test-load:
	$(PYTHON) -m pytest tests/test_load_concurrent_search.py -v --tb=short
...
api:
	$(PYTHON) -m uvicorn api.main:app --reload --host 0.0.0.0 --port 8000 --log-level info --proxy-headers
...
health:
	$(PYTHON) scripts/selfheal.py --report
