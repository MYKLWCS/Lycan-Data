...
async def test_sherlock_found_results():
    ...
    with patch("modules.crawlers.username_sherlock._check_sherlock_installed", new=AsyncMock(return_value=True)), \
         patch("modules.crawlers.username_sherlock._run_sherlock", new=AsyncMock(side_effect=fake_run)):
...
async def test_sherlock_timeout():
    ...
    with patch("modules.crawlers.username_sherlock._check_sherlock_installed", new=AsyncMock(return_value=True)), \
         patch("modules.crawlers.username_sherlock._run_sherlock", new=AsyncMock(side_effect=timeout_run)):
...
async def test_sherlock_not_installed():
    ...
    with patch("modules.crawlers.username_sherlock._check_sherlock_installed", new=AsyncMock(return_value=False)):
        result = await crawler.scrape("someuser")
...
async def test_sherlock_subprocess_args():
    ...
    mock_exec.assert_called_once_with(
        "sherlock", "targetuser", "--print-found", "--no-color", "--timeout", "10",
