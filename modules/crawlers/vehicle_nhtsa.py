...
_RECALLS_URL = "https://api.nhtsa.gov/recalls/recallsByVehicle?make={make}&model={model}&modelYear={year}"
...
def _parse_decode_results(results: list[dict]) -> dict[str, Any]:
    ...
    for item in results:
        ...
        if var_name in _WANTED_VARS and value and value.lower() not in ("not applicable", "null", "none"):
    ...
    async def scrape(self, identifier: str) -> CrawlerResult:
        ...
        if resp.status_code != 200:
            return self._result(
                ...
                error=f"http_{resp.status_code}", vin=vin, recalls=[],
            )
        ...
        try:
            ...
            return self._result(identifier, found=False, error="json_parse_error", vin=vin, recalls=[])
