"""
crypto_bscscan.py — BNB Smart Chain wallet lookup via BscScan API.
...
def _wei_to_bnb(wei: int | float) -> float:
    """Convert wei integer to BNB float."""
    return int(wei) / _WEI
        ...
        out.append(
            {
                ...
                "from": tx.get("from", ""),
                "to": tx.get("to", ""),
                "value": _wei_to_bnb(tx.get("value", 0)),
...
@register("crypto_bscscan")
class CryptoBscscanCrawler(HttpxCrawler):
    """
    Queries BscScan for BNB wallet balance and recent transactions.

    identifier: BSC wallet address (0x...)
    """

    platform = "crypto_bscscan"
    ...
    async def scrape(self, identifier: str) -> CrawlerResult:
        ...
        api_key: str = getattr(settings, "bscscan_api_key", "") or "YourApiKeyToken"
        ...
        balance_bnb = _wei_to_bnb(balance_data.get("result", 0))
