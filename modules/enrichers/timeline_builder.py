...
class TimelineBuilder:
                ...
                events.append(
                    {
                        ...
                        "description": (f"Industry: {r.industry}" if r.industry else None),
                        "location": r.location,
    ...
    async def _events_from_addresses(
        ...
        result = await session.execute(
            select(Address).where(Address.person_id == person_id).order_by(Address.created_at.asc())
        )
        records = result.scalars().all()
        events: list[dict] = []
        ...
        for r in records:
            ...
            if not move_date:
                continue
            parts = [p for p in [r.street, r.city, r.state_province, r.country] if p]
    ...
    async def _events_from_properties(
        ...
        result = await session.execute(select(Property).where(Property.person_id == person_id))
        records = result.scalars().all()
        events: list[dict] = []
    ...
    async def _events_from_adverse_media(
        ...
        for r in records:
            ...
            _severity_confidence = {"critical": 0.90, "high": 0.80, "medium": 0.65, "low": 0.50}
            events.append(
                {
    ...
    async def _events_from_pep(self, session: AsyncSession, person_id: uuid.UUID) -> list[dict]:
        result = await session.execute(
    ...
    async def _events_from_breaches(
        ...
        for r in records:
            ...
            if not breach_date:
                continue
            fields_str = ", ".join(r.exposed_fields[:5]) if r.exposed_fields else "unknown data"
            events.append(
                {
    ...
    async def _events_from_travel(self, session: AsyncSession, person_id: uuid.UUID) -> list[dict]:
        try:
