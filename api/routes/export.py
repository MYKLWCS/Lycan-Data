...
async def export_person_json(person_id: uuid.UUID, db: AsyncSession = DbDep):
    ...
    identifiers = (await db.scalars(select(Identifier).where(Identifier.person_id == person_id))).all()
    socials = (await db.scalars(select(SocialProfile).where(SocialProfile.person_id == person_id))).all()
    payload = {
        "person": {"id": str(person.id), "full_name": person.full_name, "dob": str(person.dob) if person.dob else None,
                   "nationality": person.nationality, "risk_score": person.risk_score, "meta": person.meta},
        "aliases": [{"name": a.full_name, "confidence": a.confidence} for a in aliases],
        "identifiers": [{"type": i.identifier_type, "value": i.value, "platform": i.platform} for i in identifiers],
        "social_profiles": [{"platform": s.platform, "username": s.username, "url": s.profile_url} for s in socials],
    }
