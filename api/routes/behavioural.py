...
async def get_behavioural_profile(person_id: uuid.UUID, db: AsyncSession = DbDep):
    ...
    signals = (
        await db.scalars(select(BehaviouralSignal).where(BehaviouralSignal.profile_id == row.id))
    ).all()
    return {
        "profile": {
            "id": str(row.id),
            "person_id": str(row.person_id),
            "gambling_score": row.gambling_score,
            "drug_signal_score": row.drug_signal_score,
